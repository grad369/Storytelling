    ...
    [newlyVisibleItems enumerateObjectsUsingBlock:^(UICollectionViewLayoutAttributes *item, NSUInteger idx, BOOL *stop) {
        if (item.representedElementKind == nil) {
            ...
            if (!CGPointEqualToPoint(CGPointZero, touchLocation)) {
                CGFloat yDistanceFromTouch = touchLocation.y - springBehaviour.anchorPoint.y;
                CGFloat xDistanceFromTouch = 0;
                CGFloat scrollResistance = (fabsf(yDistanceFromTouch) + xDistanceFromTouch) / 750.0f;
                
                if (self.latestDelta < 0 && yDistanceFromTouch > 0) {
                    center.y += MAX(self.latestDelta, self.latestDelta*scrollResistance);
                }
                else if (self.latestDelta >= 0 && yDistanceFromTouch <= 0) {
...
-(BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    ...
    delta += newBounds.size.height - scrollView.bounds.size.height;
    ...
    [self.dynamicAnimator.behaviors enumerateObjectsUsingBlock:^(UIAttachmentBehavior *springBehaviour, NSUInteger idx, BOOL *stop) {
        CGFloat yDistanceFromTouch = touchLocation.y - springBehaviour.anchorPoint.y;
        CGFloat xDistanceFromTouch = 0;
        CGFloat scrollResistance = (fabsf(yDistanceFromTouch) + xDistanceFromTouch) / 750;
        
        UICollectionViewLayoutAttributes *item = [springBehaviour.items firstObject];
        CGPoint center = item.center;
        if (delta < 0 && yDistanceFromTouch > 0) {
            center.y += MAX(delta, delta*scrollResistance);
        }
        else if (delta >= 0 && yDistanceFromTouch <= 0){
