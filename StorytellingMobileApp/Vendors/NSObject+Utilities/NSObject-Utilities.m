...
// Perform the selector if possible, on main thread
- (void) tryPerformSelectorOnMainThread: (SEL) aSelector withObject: (id) object1
{
	if ([self respondsToSelector:aSelector]) 
        [self performSelectorOnMainThread:aSelector withObject:object1 waitUntilDone:NO];
}
- (void) tryPerformSelectorOnMainThread: (SEL) aSelector
{
	[self tryPerformSelectorOnMainThread:aSelector withObject:nil];
}
